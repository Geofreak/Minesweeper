...
class mineSweeper {
  ...
  async showMessage() {
    ...
    if (winner) {
      // Platzhalter anzeigen, dann Servernachricht nachladen
      this.updateFeedback('Lade Nachricht…');
      var message = await fetchWinMessage();
      this.updateFeedback(message);
    } else {
      this.updateFeedback('Boom! Du hast verloren.');
    }
  }
}
